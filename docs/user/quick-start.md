...
For long local `agent` smoke runs against this server, Cursor CLI **2026-08-26** supports persistent sessions (`agent persist` / `/detach` / `agent persist attach`) so a dropped terminal does not kill the MCP client mid-check. Desktop pin **3.21.13** (feature changelog still **3.11** through **2026-09-10**).
