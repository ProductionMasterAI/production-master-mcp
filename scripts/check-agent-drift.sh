...
# CLAUDE.md must reference AGENTS.md — canonical form is line 1 == "@AGENTS.md"
if [[ -f "$ROOT/CLAUDE.md" ]]; then
  FIRST_LINE="$(head -n1 "$ROOT/CLAUDE.md" | tr -d '[:space:]')"
  if [[ "$FIRST_LINE" != "@AGENTS.md" ]] && ! grep -qE 'AGENTS\.md' "$ROOT/CLAUDE.md"; then
    err "CLAUDE.md must reference AGENTS.md (line 1 should be '@AGENTS.md')"
  fi
fi
